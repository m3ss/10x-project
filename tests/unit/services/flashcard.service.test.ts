  ...
  describe("createFlashcards", () => {
    ...
    describe("when validation fails", () => {
      ...
      it("should throw error for invalid source value", async () => {
        ...
        // Act & Assert
        await expect(service.createFlashcards(mockFlashcards, mockUserId)).rejects.toThrow("Invalid source value");
      });
  ...
  describe("getFlashcards", () => {
    describe("when operation succeeds", () => {
      ...
      it("should return correct pagination data", async () => {
        // Arrange
        const mockFlashcards: FlashcardDto[] = Array(5)
          .fill({})
          .map((_, i) => ({
            id: i + 1,
            front: `Question ${i + 1}`,
    ...
    describe("when validation fails", () => {
      it("should throw error when page < 1", async () => {
        // Act & Assert
        await expect(service.getFlashcards(mockUserId, 0, 20)).rejects.toThrow("Page number must be at least 1");
      });

      it("should throw error when limit < 1", async () => {
        // Act & Assert
        await expect(service.getFlashcards(mockUserId, 1, 0)).rejects.toThrow("Limit must be between 1 and 100");
      });

      it("should throw error when limit > 100", async () => {
        // Act & Assert
        await expect(service.getFlashcards(mockUserId, 1, 101)).rejects.toThrow("Limit must be between 1 and 100");
      });
    });
    ...
    describe("edge cases", () => {
      it("should handle last page with partial results", async () => {
        // Arrange
        const mockFlashcards: FlashcardDto[] = Array(3)
          .fill({})
          .map((_, i) => ({
            id: i + 1,
            front: `Question ${i + 1}`,
  ...
  describe("updateFlashcard", () => {
    ...
    describe("when operation succeeds", () => {
      it("should update front field", async () => {
        ...
        mockSupabase.from = vi.fn().mockReturnValueOnce(mockGetQuery).mockReturnValueOnce(mockUpdateQuery);
        ...
        expect(mockUpdateQuery.update).toHaveBeenCalledWith(expect.objectContaining({ front: "Updated Question" }));
      });
      ...
      it('should change source from "ai-full" to "ai-edited" when editing', async () => {
        ...
        expect(mockUpdateQuery.update).toHaveBeenCalledWith(expect.objectContaining({ source: "ai-edited" }));
      });
    ...
    describe("when validation fails", () => {
      ...
      it("should throw error when back is empty", async () => {
        ...
        // Act & Assert
        await expect(service.updateFlashcard(1, mockUserId, updates)).rejects.toThrow("Flashcard back cannot be empty");
      });
    ...
    describe("when operation fails", () => {
      it('should throw "Flashcard not found" when flashcard does not exist', async () => {
        ...
        // Act & Assert
        await expect(service.updateFlashcard(999, mockUserId, updates)).rejects.toThrow("Flashcard not found");
      });
      ...
      it('should throw "Flashcard not found" when update returns null data', async () => {
        ...
        // Act & Assert
        await expect(service.updateFlashcard(1, mockUserId, updates)).rejects.toThrow("Flashcard not found");
      });
    });
